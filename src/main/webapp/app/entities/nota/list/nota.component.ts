...
import { EntityArrayResponseType, NotaService } from '../service/nota.service';
...
export class NotaComponent implements OnInit {
  ...
  trackId = (item: INota): number => this.notaService.getNotaIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.notas.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: INota[] | null): INota[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const notasNew = this.notas();
      if (data) {
        for (const d of data) {
          if (notasNew.some(op => op.id === d.id)) {
            notasNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.notaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
