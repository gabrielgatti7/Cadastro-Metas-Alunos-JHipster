...
import static com.gabriel.cadastrodemetas.domain.MetaAsserts.*;
...
import com.gabriel.cadastrodemetas.service.MetaService;
...
class MetaResourceIT {
    ...
    public static Meta createEntity(EntityManager em) {
        Meta meta = new Meta().valor(DEFAULT_VALOR).area(DEFAULT_AREA);
        // Add required entity
        Aluno aluno;
    ...
    public static Meta createUpdatedEntity(EntityManager em) {
        Meta updatedMeta = new Meta().valor(UPDATED_VALOR).area(UPDATED_AREA);
        // Add required entity
        Aluno aluno;
        if (TestUtil.findAll(em, Aluno.class).isEmpty()) {
